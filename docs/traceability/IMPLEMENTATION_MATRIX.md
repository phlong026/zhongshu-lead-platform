...
| COLLAB | 负责人内部交接/回收、员工任务隔离、管理端当前跟进人展示、超级管理员按理由审计交接历史 | `apps/api/src/services/company_assignment_v12.py`、`apps/api/src/services/lead_export_v12.py`、`apps/api/src/routers/v12_dispatch.py` | `apps/h5/public/v12-workbench.html`、`apps/admin/public/v12-operations.html` | `apps/api/tests/test_company_assignment_collaboration.py`、`apps/api/tests/test_customer_feedback_831_exports.py`、`apps/api/tests/test_customer_feedback_831_admin_ui.py`：自动化通过 | 待真实负责人/员工移动端交接、运营当前跟进人展示和审计抽样 UAT |
...
| FEEDBACK-831 | 8 月 31 日六项反馈：全国服务区域关键词直加；截图/录音任一证据且多文件部分成功可提交并保留逐文件结果；管理端展示当前加盟商跟进人；测试客资先预览影响再以完整 ID 和客户名受控删除，错派撤回重派时同步关闭退回草稿；客资完整信息导出；公海池按日期、录入人等当前筛选或全部导出。错派保留派发、积分、奖励、通知和审计事实，公海池仍不开放派发/领取/退回动作 | `apps/api/src/routers/master_data.py`、`apps/api/src/routers/v12_insights.py`、`apps/api/src/routers/v12_lead_supply.py`、`apps/api/src/routers/v12_public_pool.py`、`apps/api/src/routers/v12_returns.py`、`apps/api/src/services/china_regions.py`、`apps/api/src/services/lead_export_v12.py`、`apps/api/src/services/lead_supply_v12.py`、`apps/api/src/services/public_pool_v12.py`、`apps/api/src/services/return_v12.py`、`migrations/versions/0016_lead_test_flag.py` | `apps/admin/public/v12-operations.js`、`apps/h5/public/v12-workbench.js` | `apps/api/tests/test_customer_feedback_831_admin_ui.py`、`apps/api/tests/test_customer_feedback_831_exports.py`、`apps/api/tests/test_customer_feedback_831_lead_governance.py`、`apps/api/tests/test_customer_feedback_831_return_evidence.py`、`apps/api/tests/test_v12_migration.py` | 待真实运营/超级管理员/负责人角色、生产 PostgreSQL、对象存储、微信 WebView 与导出文件人工 UAT；当前仅完成代码与自动化，不代表已部署 |
...
- 在线支付、微信支付或支付宝；
- 自动、随机、轮询、权重、竞价或抢单派发；
- 向其他公司公开加盟商内部员工信息，或向平台开放员工分配/回收历史；平台仅显示当前跟进人；
- 微信小程序或独立 App；
