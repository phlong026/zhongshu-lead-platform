...
所有作业均具备业务幂等键；通知失败不会回滚已经完成的派发、积分或审核事务。对象存储清理失败会保持 FAILED 并持续按封顶退避重试，不会恢复已删除的测试业务数据；连续失败 5 次后会记录 error 日志。Bucket、Endpoint 或 Region 与任务入队时不一致时会拒绝删除，必须切回原存储目标处理。
