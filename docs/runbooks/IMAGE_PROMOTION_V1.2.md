...
docker tag '<scan-subject.image_ref>' \
  'registry.example.com/zhongshu-lead-platform:1.2.3'

docker push 'registry.example.com/zhongshu-lead-platform:1.2.3'
```

推送完成后获取 registry 返回的 manifest digest，并形成最终不可变引用：

```text
registry.example.com/zhongshu-lead-platform:1.2.3@sha256:<registry-manifest-digest>
```
...
```bash
docker pull 'registry.example.com/zhongshu-lead-platform:1.2.3@sha256:<digest>'
```
