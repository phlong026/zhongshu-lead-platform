...
```bash
CONFIRM_RESTORE=YES sh scripts/restore_postgres.sh backups/postgres/zhongshu-YYYYMMDDTHHMMSSZ.dump
```
...
CONFIRM_RESTORE=YES sh scripts/restore_private_storage.sh backups/private-storage/private-storage-*.tar.gz
```
