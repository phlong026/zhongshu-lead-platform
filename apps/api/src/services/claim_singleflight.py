...
def run_claim_singleflight(
    ...
    *,
    ...
    """Collapse concurrent same-assignment claims inside one API worker.

    The leader owns the existing authoritative database transaction. Followers
    wait for its plain response payload and can release any read-only database
    transaction before waiting. No claim business rule is duplicated here.
    """
