...
def test_singleflight_does_not_mix_different_company_assignment_keys() -> None:
    barrier = Barrier(2)
    lock = Lock()
    executions: list[str] = []
    ...
    keys = ["company-a:assignment-1", "company-b:assignment-1"]
...
def test_completed_failure_is_removed_and_next_request_can_retry() -> None:
    executions = 0
    ...
    with pytest.raises(RuntimeError):
        run_claim_singleflight("company-a:assignment-retry-cleanup", fail_once)

    payload, coalesced = run_claim_singleflight(
        "company-a:assignment-retry-cleanup",
        lambda: {"ok": True},
    )
