...
def test_production_compose_requires_reviewed_image_and_disables_implicit_migration() -> None:
    ...
    assert "ARG APP_VERSION=1.2.2" in dockerfile
...
def test_image_reference_parser_requires_exact_version_tag() -> None:
    assert image_tag("registry.example.com/app:1.2.2@sha256:" + "a" * 64) == "1.2.2"
...
def test_scan_subject_image_id_is_strict_and_inspect_uses_docker_identity(
    ...
    class Result:
        ...
        stdout = json.dumps(
            {
                "Id": expected,
                "Config": {"Labels": {"org.opencontainers.image.version": "1.2.2"}},
            }
        )
        stderr = ""
    ...
    version, actual, descriptor, config, error = inspect_image_metadata(
        "docker", "registry.example.com/app:1.2.2@sha256:digest"
    )
    ...
    version, actual, descriptor, config, error = inspect_image_metadata(
        "docker", "registry.example.com/app:1.2.2"
    )
...
def test_ci_contains_postgres_browser_dependency_evidence_and_packaging_gates() -> None:
    ...
    assert "--version V1.2.2-rc" in pr_workflow
    assert "--version V1.2.2" in release_workflow
