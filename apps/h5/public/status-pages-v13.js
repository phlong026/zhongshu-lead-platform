...
const ZS_STATUS_LABELS={V1:'普通加盟商',V2:'重点加盟商',V3:'核心加盟商',OLD_RENOVATION:'旧房改造',SELF_BUILD:'农村自建房',INTERIOR:'室内装修',VILLA_DECORATION:'别墅装修',LEAD_SUPPLIER:'供客能力',LEAD_RECEIVER:'接收客资能力'};
const zsStatusLabel=(value,fallback)=>ZS_STATUS_LABELS[value]||fallback;
const zsStatusRecordCode=(value,prefix='记录')=>{const text=String(value??'').replace(/-/g,'');return text?`${prefix}-${text.slice(-8).toUpperCase()}`:'已生成'};
...
async function zsLoadInvitePreview(invite,card){
  ...
  try{
    ...
    const x=p.data;const regionCount=(x.region_codes||[]).length;const capabilities=(x.capability_codes||[]).map(code=>zsStatusLabel(code,'其他服务'));card.hidden=false;zsSetSafeHtml(card, `<h3>请确认是否绑定到【${zsStatusEsc(x.company_name)}】</h3><dl class="zs-v13-invite-grid"><dt>负责人</dt><dd>${zsStatusEsc(x.owner_name||'加盟商负责人')}</dd><dt>服务地区</dt><dd>${zsStatusEsc(regionCount?`已配置 ${regionCount} 个服务地区`:'以公司档案为准')}</dd><dt>业务范围</dt><dd>${zsStatusEsc([...new Set(capabilities)].join('、')||'以公司档案为准')}</dd><dt>会员等级</dt><dd>${zsStatusEsc(zsStatusLabel(x.level_code,'普通加盟商'))}</dd><dt>邀请有效期至</dt><dd>${zsStatusEsc(x.expires_at?new Date(x.expires_at).toLocaleString('zh-CN',{year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'}):'--')}</dd></dl><p class="help">请确认以上公司信息无误，勾选服务规则后点击“微信授权登录”完成绑定。</p>`);
...
function zsRenderReturnSuccess(){let data={};try{data=JSON.parse(sessionStorage.getItem('zs:return-success')||'{}')}catch{}zsRenderState({kind:'success',icon:'circle-check',title:'退回申请已提交',message:'管理员将核验聊天截图和电话录音。审核通过后，领取时实际扣除的积分将返还。',detail:[['退回编号',zsStatusRecordCode(data.id,'TH')],['客资',data.lead||'当前客资'],['申请返还',data.points?`${data.points} 积分`:'以审核结果为准'],['当前状态','待审核']],primary:['查看退回进度','leads?status=RETURN_PENDING'],secondary:['返回我的客资','leads']});}
