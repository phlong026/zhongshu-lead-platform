...
function zsPatchAuthPage(){
  ...
  if(logo){logo.querySelector('h1').textContent='众墅之家';logo.querySelector('p').textContent='客资审核 · 派发 · 积分管理';}
  const hero=document.createElement('section');hero.className='zs-v13-auth-hero';hero.innerHTML='<h2>欢迎使用加盟商客资平台</h2><p>微信内打开，无需安装。授权后可直接进入有权限的客资页面。</p><div class="zs-v13-auth-tags"><span class="zs-v13-auth-tag">微信 OAuth</span><span class="zs-v13-auth-tag info">公司级权限</span></div>';
  ...
  const foot=document.createElement('p');foot.className='zs-v13-auth-foot';foot.textContent='所有关键操作均从公众号消息深链进入，H5 首页仅作为备用入口。';page.appendChild(foot);
...
async function zsLoadInvitePreview(invite,card){
  try{const r=await fetch(`${ZS_STATUS_API}/auth/invites/preview?invite=${encodeURIComponent(invite)}`,{credentials:'include'});const p=await r.json();if(!r.ok||p.code!=='OK')throw new Error(p.message||'邀请已失效');const x=p.data;card.hidden=false;card.innerHTML=`<h3>${zsStatusEsc(x.company_name)}</h3><dl class="zs-v13-invite-grid"><dt>负责人</dt><dd>${zsStatusEsc(x.owner_name||'加盟商负责人')}</dd><dt>服务地区</dt><dd>${zsStatusEsc((x.region_codes||[]).join('、')||'以公司档案为准')}</dd><dt>业务范围</dt><dd>${zsStatusEsc((x.capability_codes||[]).join('、')||'以公司档案为准')}</dd><dt>会员等级</dt><dd>${zsStatusEsc(x.level_code||'V1')}</dd></dl>`;
  }catch(e){card.hidden=false;card.innerHTML=`<h3>邀请无法使用</h3><p>${zsStatusEsc(e.message)}</p>`;const btn=document.querySelector('#wechat-login');if(btn)btn.disabled=true;}
}
...
function zsPatchInvalidLink(){const title=document.querySelector('.topbar .brand>div');const empty=document.querySelector('main.content .empty');if(!empty||!title||!/(链接失效|页面加载失败)/.test(title.textContent||''))return;if(empty.dataset.zsSecure==='1')return;empty.dataset.zsSecure='1';empty.innerHTML='<div class="zs-v13-state-icon">⌁</div><h2>该链接已失效</h2><p>客资可能已被领取、回收、释放，或当前账号无权访问。为保护客户隐私，本页面不展示任何客资摘要。</p><button class="btn btn-primary" data-zs-status-route="leads">返回客资列表</button>';
}
function zsStatusPatch(){zsPatchAuthPage();zsPatchLinkLanding();zsPatchInvalidLink();}
