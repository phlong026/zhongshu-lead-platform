...
const badge=v=>`<span class="wb-status ${['APPROVED','CLAIMED','SETTLED','COMPLETED','READ'].includes(v)?'ok':['REJECTED','CANCELLED','REVERSED'].includes(v)?'bad':'warn'}">${esc(LABEL[v]||v||'--')}</span>`;
...
async function home(){const d=await api('/v1.2/reports/own');shell(`<section class="wb-hero"><h1>今天的客资工作</h1><p>供应、领取、退回、奖励和消息集中处理。</p><div class="wb-kpis"><div class="wb-kpi"><b>${d.supplier_leads.total}</b><span>供应客资</span></div><div class="wb-kpi"><b>${d.received_assignments.total}</b><span>接收客资</span></div><div class="wb-kpi"><b>${d.returns.total}</b><span>退回申诉</span></div><div class="wb-kpi"><b>${d.supplier_rewards.points}</b><span>奖励积分</span></div><div class="wb-kpi"><b>${d.unread_notifications||0}</b><span>未读消息</span></div></div></section><div class="wb-grid">${can('supplier.lead.manage')?`<div class="wb-card wb-action-card" data-go="leads"><div class="wb-action-icon">${icon('inbox')}</div><b>供应客资</b><span class="wb-muted">提交与查看初审状态</span></div>`:''}${can('assignment.own.read')?`<div class="wb-card wb-action-card" data-go="assignments"><div class="wb-action-icon">${icon('hand-claim')}</div><b>接收客资</b><span class="wb-muted">领取后查看完整联系方式</span></div>`:''}${can('company.profile.manage')?`<div class="wb-card wb-action-card" data-go="profile"><div class="wb-action-icon">${icon('building')}</div><b>加盟商设置</b><span class="wb-muted">申请客资供应、接收资格与服务区域</span></div>`:''}${can('return.own.manage')?`<div class="wb-card wb-action-card" data-go="returns"><div class="wb-action-icon">${icon('rotate-ccw')}</div><b>退回申诉</b><span class="wb-muted">提交说明和一份有效证据</span></div>`:''}${can('supplier.reward.own.read')?`<div class="wb-card wb-action-card" data-go="rewards"><div class="wb-action-icon">${icon('award')}</div><b>供应奖励</b><span class="wb-muted">查看待结算和已到账奖励</span></div>`:''}${canView('notifications')?`<div class="wb-card wb-action-card" data-go="notifications"><div class="wb-action-icon">${icon('bell')}</div><b>消息中心</b><span class="wb-muted">${d.unread_notifications||0} 条未读消息</span></div>`:''}</div>`);document.querySelectorAll('[data-go]').forEach(x=>x.onclick=()=>go(x.dataset.go))}
...
async function returns(){const d=await api(`/v1.2/returns?page=${S.page}&page_size=20`);const list=(d.items||[]).map(x=>item(`退回申诉 · ${LABEL[x.reason_code]||x.reason_code}`,x.status,`<p>提交时间 ${fmt(x.submitted_at||x.created_at)}</p><p>派发单 ${esc(x.assignment_id)}</p>`,`<button class="wb-btn" data-return="${x.id}">查看进度</button>`)).join('');shell(`<div class="wb-card-head"><div><h2>退回申诉</h2><p>发起申诉后，平台会根据说明、证据和电话核验结果进行审核。</p></div></div><div class="wb-list">${list||'<div class="wb-empty">暂无退回申诉</div>'}</div>`);document.querySelectorAll('[data-return]').forEach(b=>b.onclick=()=>returnDetail(b.dataset.return));if(S.id){const id=S.id;S.id='';returnDetail(id)}}
async function returnDetail(id){const x=await api(`/v1.2/returns/${id}`),verification=x.verification||{};openSheet('申诉详情',`<div class="wb-detail-grid">${[['申诉编号',x.id],['派发单',x.assignment_id],['处理状态',LABEL[x.status]||x.status],['退回原因',LABEL[x.reason_code]||x.reason_code],['电话核验',verification.status?LABEL[verification.status]||verification.status:'待安排'],['核验结论',verification.conclusion?LABEL[verification.conclusion]||verification.conclusion:'尚未提交'],['申诉截止',fmt(x.appeal_deadline_at)],['最终结果',x.final_decision_reason||'待审核']].map(([a,b])=>`<div class="wb-detail"><small>${a}</small><b>${esc(b||'--')}</b></div>`).join('')}</div><div class="wb-card"><h3>申诉说明</h3><p class="wb-muted">${esc(x.description||'暂无说明')}</p></div>`)}
function rewardExplanation(x){if(x.status==='OBSERVING')return `奖励正在确认中，预计结算时间为 ${fmt(x.reward_due_at)}。`;if(x.status==='FROZEN')return `奖励暂缓结算。${x.exception_reason||'平台复核完成后会更新进度。'}`;if(x.status==='SETTLED')return `奖励已于 ${fmt(x.settled_at)} 结算到账。`;if(x.status==='WAITING_CLAIM')return '客资被领取后会进入奖励确认。';if(x.status==='CANCELLED')return `本次奖励已取消。${x.exception_reason||''}`;if(x.status==='REVERSED')return `本次奖励已调整。${x.exception_reason||''}`;return '奖励进度以当前页面显示为准。'}
async function rewards(){const d=await api(`/v1.2/supplier-rewards?page=${S.page}&page_size=20`);const sum=d.summary||{};const list=(d.items||[]).map(x=>item(`${x.reward_points} 奖励积分`,x.status,`<p>当前进度：${esc(LABEL[x.status]||x.status)}</p><p>预计结算：${fmt(x.reward_due_at)}</p>`,`<button class="wb-btn" data-reward="${x.id}">查看说明</button>`)).join('');shell(`<section class="wb-hero"><h1>供应商奖励</h1><p>客资被领取后，可在这里查看奖励确认和到账进度。</p><div class="wb-kpis"><div class="wb-kpi"><b>${sum.total_count||0}</b><span>奖励笔数</span></div><div class="wb-kpi"><b>${sum.settled_points||0}</b><span>已结算积分</span></div><div class="wb-kpi"><b>${sum.observing_points||0}</b><span>确认中积分</span></div><div class="wb-kpi"><b>${sum.frozen_points||0}</b><span>暂缓积分</span></div></div></section><div class="wb-list">${list||'<div class="wb-empty">暂无奖励记录，客资被领取后会在这里展示进度。</div>'}</div>`);document.querySelectorAll('[data-reward]').forEach(b=>b.onclick=()=>rewardDetail(b.dataset.reward));if(S.id){const id=S.id;S.id='';rewardDetail(id)}}
async function rewardDetail(id){const x=await api(`/v1.2/supplier-rewards/${id}`),rule=x.rule_snapshot||{},ratio=(Number(rule.ratio_bps||x.reward_ratio_bps||0)/100).toFixed(2).replace(/\.00$/,'');openSheet('奖励详情',`<div class="wb-detail-grid">${[['当前进度',LABEL[x.status]||x.status],['奖励积分',x.reward_points],['对应客资积分',x.claim_points],['奖励比例',`${ratio}%`],['进入确认',fmt(x.observed_at)],['预计结算',fmt(x.reward_due_at)],['实际到账',fmt(x.settled_at)]].map(([a,b])=>`<div class="wb-detail"><small>${a}</small><b>${esc(b??'--')}</b></div>`).join('')}</div><div class="wb-card"><h3>奖励说明</h3><p class="wb-muted">${esc(rewardExplanation(x))}</p></div><div class="wb-notice">本笔奖励按客资被领取时生效的比例计算，不受后续调整影响。</div>`)}
