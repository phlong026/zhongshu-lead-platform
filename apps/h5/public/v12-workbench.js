...
function profileSecurity(){return `<section class="wb-card wb-account-security"><div class="wb-card-head"><h2>账户与安全</h2></div><div class="wb-account-actions"><button class="wb-account-action" id="profile-username" type="button"><i>${icon('user')}</i><span><b>登录账号</b><small>${esc(S.me?.username||'--')}</small></span><em>${icon('chevron-right')}</em></button><button class="wb-account-action" id="profile-password" type="button"><i>${icon('key-round')}</i><span><b>登录密码</b><small>修改密码</small></span><em>${icon('chevron-right')}</em></button></div></section>`}
...
function changeProfilePassword(){openSheet('修改登录密码',`<form class="wb-form" id="profile-password-form"><div class="wb-field"><label for="profile-password-current">当前密码</label><input class="wb-input" id="profile-password-current" type="password" autocomplete="current-password" minlength="8" maxlength="128" required></div><div class="wb-field"><label for="profile-password-next">新密码</label><input class="wb-input" id="profile-password-next" type="password" autocomplete="new-password" minlength="8" maxlength="128" required></div><div class="wb-field"><label for="profile-password-confirm">确认新密码</label><input class="wb-input" id="profile-password-confirm" type="password" autocomplete="new-password" minlength="8" maxlength="128" required></div><button class="wb-btn primary" type="submit" id="profile-password-submit">保存</button></form>`,()=>{const form=document.querySelector('#profile-password-form'),submit=document.querySelector('#profile-password-submit');form.onsubmit=async event=>{event.preventDefault();const current_password=document.querySelector('#profile-password-current').value,new_password=document.querySelector('#profile-password-next').value,confirm_password=document.querySelector('#profile-password-confirm').value;if(new_password.length<8){toast('新密码至少 8 位',true);return}if(new_password!==confirm_password){toast('两次输入的新密码不一致',true);return}submit.disabled=true;try{await api('/auth/change-password',{method:'POST',body:JSON.stringify({current_password,new_password})});closeSheet();toast('密码已更新');await profile()}catch(error){submit.disabled=false;toast(error.message,true)}}})}
async function profile(){
  ...
  if(!can('company.profile.manage')){
    shell(identity+security+profileLogout());
    bindProfileSecurity();
    return;
  }
  ...
  shell(identity+security+`<section class="wb-card" id="service-areas"><div class="wb-card-head"><h2>经营区域</h2><span class="wb-region-count">${approvedAreas} 个</span></div><div class="wb-list">${areaCards}</div></section>`+profileLogout());
  bindProfileSecurity();
}
...
async function returns(){const d=await api(`/v1.2/returns?page=${S.page}&page_size=20`);const list=(d.items||[]).map(x=>item(`退回申诉 · ${readableLabel(x.reason_code,'其他原因')}`,x.status,`<p>提交时间 ${fmt(x.submitted_at||x.created_at)}</p><p>派发编号 ${esc(recordCode(x.assignment_id,'PF'))}</p>`,`<button class="wb-btn" data-return="${x.id}">查看进度</button>`)).join('');shell(`<section class="wb-page-head"><h1>退回申诉</h1></section><div class="wb-list">${list||'<div class="wb-empty">暂无退回申诉</div>'}</div>`);document.querySelectorAll('[data-return]').forEach(b=>b.onclick=()=>returnDetail(b.dataset.return));if(S.id){const id=S.id;S.id='';returnDetail(id)}}
