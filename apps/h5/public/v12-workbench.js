...
const LABEL={PENDING:'待审核',PENDING_REVIEW:'待初审',READY_DISPATCH:'待派发',PENDING_CLAIM:'待领取',CLAIMED:'已领取',FOLLOWING:'跟进中',COMPLETED:'已完成',UNCONTACTED:'未联系',CONTACTED:'已联系',INTERESTED:'有意向',NOT_INTERESTED:'无意向',DEAL:'已成交',INVALID:'无效客资',SUBMITTED:'已提交',VERIFYING:'核验中',REVIEWING:'待终审',NEED_MORE_EVIDENCE:'待补证',APPROVED:'已通过',REJECTED:'已驳回',OBSERVING:'观察期',FROZEN:'已冻结',SETTLED:'已结算',CANCELLED:'已取消',REVERSED:'已撤销',WAITING_CLAIM:'待领取',READ:'已读',UNREAD:'未读',CLEAR:'无重复',DUPLICATE:'疑似重复',EMPTY_NUMBER:'空号或停机',OUT_OF_SERVICE_REGION:'超出服务区域',DUPLICATE_TO_RECEIVER:'接收方重复客户',NON_HOUSING_CONSULTATION:'非建房装修咨询',ASSIGNED:'待处理',IN_PROGRESS:'核验中',SUPPORT_RETURN:'支持退回',DOES_NOT_SUPPORT_RETURN:'不支持退回',INCONCLUSIVE:'信息不足'};
...
async function leadDetail(id){const x=await api(`/v1.2/supplier/leads/${id}`);openSheet('客资详情',`<div class="wb-detail-grid">${[['客资编号',x.id],['客户',x.customer_name],['手机号',x.phone_masked],['服务地区',`${x.city||''} ${x.district||''}`],['处理状态',LABEL[x.status]||x.status],['初审结果',LABEL[x.review_status]||x.review_status],['重复检查',LABEL[x.duplicate_status]||x.duplicate_status]].map(([a,b])=>`<div class="wb-detail"><small>${a}</small><b>${esc(b||'--')}</b></div>`).join('')}</div><div class="wb-card"><h3>客户需求</h3><p class="wb-muted">${esc(x.need_summary||'暂无说明')}</p></div>${x.review_note?`<div class="wb-notice">平台说明：${esc(x.review_note)}</div>`:''}`)}
async function assignments(){const d=await api(`/v1.2/assignments?page=${S.page}&page_size=20`);const list=(d.items||[]).map(x=>item(x.customer_name||x.lead?.customer_name||'客户',x.status,`<p>${esc(x.phone||x.phone_masked||'领取后查看')} · ${esc(x.city||x.lead?.city||'')}</p><p>领取所需 ${x.points_price||0} 积分 · 请在 ${fmt(x.claim_deadline_at)} 前领取</p>`,`<button class="wb-btn" data-assignment="${x.id}">详情</button>${x.status==='PENDING_CLAIM'&&can('assignment.own.claim')?`<button class="wb-btn primary" data-claim="${x.id}">领取</button>`:''}`)).join('');shell(`<div class="wb-card-head"><div><h2>接收客资</h2><p>领取前仅显示脱敏号码；领取成功后扣减对应积分并显示完整联系方式。</p></div></div><div class="wb-list">${list||'<div class="wb-empty">暂无待领取客资</div>'}</div>`);document.querySelectorAll('[data-assignment]').forEach(b=>b.onclick=()=>assignmentDetail(b.dataset.assignment));document.querySelectorAll('[data-claim]').forEach(b=>b.onclick=()=>claim(b.dataset.claim));if(S.id){const id=S.id;S.id='';assignmentDetail(id)}}
...
async function rewardDetail(id){const x=await api(`/v1.2/supplier-rewards/${id}`),rule=x.rule_snapshot||{},ratio=(Number(rule.ratio_bps||x.reward_ratio_bps||0)/100).toFixed(2).replace(/\.00$/,'');openSheet('奖励详情',`<div class="wb-detail-grid">${[['奖励编号',x.id],['奖励状态',LABEL[x.status]||x.status],['对应客资积分',x.claim_points],['预计奖励',`${x.reward_points} 积分`],['奖励比例',`${ratio}%`],['预计结算',fmt(x.reward_due_at)],['实际到账',fmt(x.settled_at)]].map(([a,b])=>`<div class="wb-detail"><small>${a}</small><b>${esc(b||'--')}</b></div>`).join('')}</div><div class="wb-notice">本笔奖励按客资被领取时生效的比例计算，不受后续调整影响。</div>`)}
