...
function loading(active='home'){app.innerHTML=shell(`<div class="skeleton"></div><div class="skeleton" style="margin-top:14px"></div>`,active);bindRoutes();}
...
function renderLogin(){
  ...
  app.innerHTML=`<section class="login-page"><div class="login-logo"><img src="./logo.png" alt="众墅之家"><h1>加盟商客资助手</h1><p>微信内打开 · 无需安装 · 点击即处理</p></div><div class="login-panel"><button class="btn btn-primary btn-block" id="wechat-login">微信授权登录</button>${invite?'<p class="help">已识别专属加盟商邀请，授权后将自动绑定。</p>':'<p class="help">正式环境请通过平台发送的邀请链接进入。</p>'}<div class="divider">本地演示环境</div><div class="form-group"><label>演示账号</label><input class="input" id="username" value="franchise_demo"></div><div class="form-group"><label>密码</label><input class="input" id="password" type="password" value="Franchise123!"></div><button class="btn btn-outline btn-block" id="demo-login">进入演示</button></div></section>`;
  ...
  document.querySelector('#demo-login').onclick=async()=>{try{await api('/auth/login',{method:'POST',body:JSON.stringify({username:document.querySelector('#username').value,password:document.querySelector('#password').value})});go('home');await route();}catch(e){toast(e.message,'error');}};
}
...
async function route(){
  ...
  try{
    ...
    go('home');
  }catch(e){console.error(e);toast(e.message||'页面加载失败','error');app.innerHTML=shell(empty('!','页面加载失败，请稍后重试'),'home');bindRoutes();}
}
