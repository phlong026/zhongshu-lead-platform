...
const BUSINESS_LABEL={PENDING_CLAIM:'待领取',CLAIMED:'待跟进',FOLLOWING:'跟进中',RETURN_PENDING:'退回审核中',RETURNED:'已退回',RELEASED:'已释放',EXPIRED:'已过期',COMPLETED:'已完成',UNCONTACTED:'未联系',CONTACTED:'已联系',INTERESTED:'有意向',NOT_INTERESTED:'无意向',DEAL:'已成交',INVALID:'需要修改',OLD_RENOVATION:'旧房改造',SELF_BUILD:'农村自建房',INTERIOR:'室内装修',V1:'普通加盟商',V2:'重点加盟商',V3:'核心加盟商',RECHARGE:'充值入账',CLAIM:'领取扣分',RETURN:'退回返分',ADJUST:'人工调整',REVERSAL:'冲正'};
const TECHNICAL_CODE=/^(?:[A-Z][A-Z0-9_]{2,}|[a-z][a-z0-9]*|[a-z0-9]+(?:[_-][a-z0-9]+)+)$/;
const businessLabel=(value,fallback='未填写')=>{const text=String(value??'').trim();if(!text)return fallback;return BUSINESS_LABEL[text]||(TECHNICAL_CODE.test(text)?fallback:text)};
const statusLabel=status=>businessLabel(status,'待确认');
const memberLevelLabel=account=>businessLabel(account?.level_code,'普通加盟商');
const packageDisplayName=packageItem=>{const raw=String(packageItem?.name||'').trim();const fallback=businessLabel(packageItem?.level_code,'充值档位');if(!raw)return fallback;const levels={V1:'普通加盟商',V2:'重点加盟商',V3:'核心加盟商'};return raw.replace(/^(V1|V2|V3)\b/i,code=>levels[code.toUpperCase()]||fallback)};
...
function shell(content, active='home', title='客资助手') {
  return `<header class="topbar"><div class="brand"><img src="./logo.png" alt="合家美宅"><div>${esc(title)}</div></div></header><main class="content">${content}</main>${nav(active)}`;
}
function nav(active){return `<nav class="bottom-nav">${[['home','home','首页'],['leads','list','客资'],['points','coins','积分'],['profile','user','我的']].map(([r,i,l])=>`<button class="nav-item ${active===r?'active':''}" data-route="${r}"><i>${icon(i)}</i>${l}</button>`).join('')}</nav>`;}
...
async function renderHome(){
  ...
  zsSetSafeHtml(app, shell(`<h1 class="page-title">您好，${esc(state.me.display_name)}</h1><p class="subtitle">${esc(account.company_name||'加盟商')} · ${esc(memberLevelLabel(account))}</p><section class="hero"><div class="row"><div><div class="eyebrow">当前可用积分</div><div class="balance">${money(account.balance)}<span> 分</span></div><div class="eyebrow">待领取预计占用 ${money(account.pending_claim_points)} 分</div></div><button class="btn btn-gold" data-route="points">积分明细</button></div></section><section class="grid metrics" aria-label="客资处理统计"><button type="button" class="metric" data-route="leads?status=PENDING_CLAIM" aria-label="待领取 ${b.pending_claim||0} 条，查看列表"><span>待领取</span><b>${b.pending_claim||0}</b></button><button type="button" class="metric" data-route="leads?status=ACTIVE" aria-label="待跟进 ${b.claimed||0} 条，查看列表"><span>待跟进</span><b>${b.claimed||0}</b></button><button type="button" class="metric" data-route="leads?status=RETURN_PENDING" aria-label="退回审核中 ${b.return_pending||0} 条，查看列表"><span>退回审核中</span><b>${b.return_pending||0}</b></button><button type="button" class="metric" data-route="leads?status=COMPLETED" aria-label="已完成 ${b.completed||0} 条，查看列表"><span>已完成</span><b>${b.completed||0}</b></button></section><section class="card"><div class="card-title"><h3>待处理客资</h3><a class="zs-icon-label" data-route="leads"><span>查看全部</span>${icon('chevron-right')}</a></div><div class="list">${assignments.items.length?assignments.items.map(assignmentCard).join(''):empty('circle-check','当前没有待处理客资')}</div></section><section class="card"><div class="card-title"><h3>领取提醒</h3></div><p class="subtitle" style="margin:0">请及时领取并跟进客资；申请退回时至少上传沟通截图或电话录音中的一种。</p></section>`,'home'));
  bindRoutes();bindAssignmentCards();
}
function assignmentCard(x){const l=x.lead_snapshot||{};return `<article class="lead-card" data-assignment="${x.id}"><div class="line"><h3>${esc(l.customer_name||'客户')}</h3><span class="badge badge-${statusClass(x.status)}">${statusLabel(x.status)}</span></div><p>${esc(l.city||'')} ${esc(l.district||'')} · ${esc(businessLabel(l.category_code,'客资'))}<br>${fmtDate(x.assigned_at)}</p><div class="line" style="margin-top:10px"><span class="price">${money(x.points_price)} 积分</span><span class="zs-icon-label"><span>查看</span>${icon('chevron-right')}</span></div></article>`;}
...
async function renderLeads(){
  ...
  const data=await api(`/dispatch/assignments?page=1&page_size=100${apiFilter?`&status=${encodeURIComponent(apiFilter)}`:''}`);
  const visibleItems=filter === 'ACTIVE' ? data.items.filter(item=>['CLAIMED','FOLLOWING'].includes(item.status)) : data.items;
  state.assignments=visibleItems;
  zsSetSafeHtml(app, shell(`<h1 class="page-title">我的客资</h1><p class="subtitle">按当前处理进度查看派发给本公司的客资</p><div class="tabs">${[['','全部'],['PENDING_CLAIM','待领取'],['ACTIVE','待跟进'],['RETURN_PENDING','退回中'],['COMPLETED','已完成']].map(([v,l])=>`<button class="tab ${filter===v?'active':''}" data-filter="${v}">${l}</button>`).join('')}</div><div class="list">${visibleItems.length?visibleItems.map(assignmentCard).join(''):empty('search','暂无符合条件的客资')}</div>`,'leads','我的客资'));
...
async function renderReturn(id){
  ...
  zsSetSafeHtml(app, shell(`${backButton()}<h1 class="page-title">申请退回</h1><p class="subtitle">截图或电话录音任一类型满足即可，平台审核通过后才返还积分。</p><div class="stepper"><div class="step active"><i>1</i>选择原因</div><div class="step active"><i>2</i>上传证据</div><div class="step"><i>3</i>提交审核</div></div><section class="card"><div class="form-group"><label>退回原因 *</label><select class="select" id="return-reason"><option value="EMPTY_NUMBER">空号/停机/无法接通</option><option value="DUPLICATE">重复客户</option><option value="REGION_WRONG">地区错误</option><option value="NON_TARGET">非目标客户</option><option value="INFO_ERROR">关键信息错误</option></select></div><div class="form-group"><label>补充说明 *</label><textarea class="textarea" id="return-description" placeholder="请具体说明核验过程、客户反馈及判断依据"></textarea></div><div class="form-group"><label>聊天/沟通截图（与录音任选一种）</label><div class="upload"><input type="file" id="screenshot-files" accept="image/png,image/jpeg,image/webp" multiple><p class="help">如选择截图，最多5张，单张不超过5MB</p></div></div><div class="form-group"><label>电话录音（与截图任选一种）</label><div class="upload"><input type="file" id="audio-file" accept="audio/*,.m4a,.mp3,.wav,.aac"><p class="help">如选择录音，请上传手机或合规工具生成的文件，单个不超过20MB</p></div></div><button class="btn btn-primary btn-block" id="submit-return">提交退回申请</button></section>`,'leads','退回申请'));bindRoutes();
...
async function submitReturn(id){
  ...
  if(description.length<3||(!screenshots.length&&!audio)){toast('请完整填写说明，并至少上传沟通截图或电话录音','error');return;}
  try{
    ...
    if(audio){const form=new FormData();form.append('evidence_type','CALL_RECORDING');form.append('file',audio);await api(`/returns/${draft.id}/evidence`,{method:'POST',body:form});}
...
async function renderPoints(){
  ...
  zsSetSafeHtml(app, shell(`<h1 class="page-title">积分中心</h1><p class="subtitle">线下付款后，由平台授权管理员人工充值积分</p><section class="hero"><div class="eyebrow">当前积分余额</div><div class="balance">${money(account.balance)}<span> 分</span></div><div class="row"><span>会员等级 ${esc(memberLevelLabel(account))}</span><span>可派发积分 ${money(account.available_for_dispatch)}</span></div></section><section class="card"><div class="card-title"><h3>充值档位参考</h3></div>${packages.map(p=>`<div class="lead-card"><div class="line"><b>${esc(packageDisplayName(p))}</b><span class="badge badge-warning">${esc(businessLabel(p.level_code,'适用加盟商'))}</span></div><p>线下实收 ¥${money(p.cash_amount_cents/100)} · 到账 ${money(p.base_points+p.bonus_points)} 积分</p></div>`).join('')}<p class="help">以上为系统配置档位，实际付款请联系平台并以线下确认结果为准。</p></section><section class="card"><div class="card-title"><h3>积分流水</h3></div><div class="list">${ledgers.items.length?ledgers.items.map(x=>`<div class="lead-card"><div class="line"><b>${esc(businessLabel(x.type,'积分变动'))}</b><span class="price" style="color:${x.delta>=0?'var(--success)':'var(--ink)'}">${x.delta>=0?'+':''}${money(x.delta)}</span></div><p>${fmtDate(x.created_at)} · 余额 ${money(x.balance_after)}</p></div>`).join(''):empty('coins','暂无积分流水')}</div></section>`,'points','积分中心'));bindRoutes();
}

async function renderNotifications(){
  loading('notifications');if(!await ensureAuth())return;
  const data=await api('/notifications?page=1&page_size=100');
  zsSetSafeHtml(app, shell(`<h1 class="page-title">消息中心</h1><p class="subtitle">查看客资、积分和审核提醒</p><div class="list">${data.items.length?data.items.map(x=>`<article class="lead-card" data-notification="${x.id}" data-link="${esc(x.deep_link||'')}"><div class="line"><h3>${esc(x.title)}</h3>${x.read_at?'':'<span class="badge badge-danger">未读</span>'}</div><p>${esc(x.body)}</p><p>${fmtDate(x.created_at)}</p></article>`).join(''):empty('bell','暂无消息')}</div>`,'profile','消息中心'));bindRoutes();document.querySelectorAll('[data-notification]').forEach(el=>el.onclick=async()=>{try{await api(`/notifications/${el.dataset.notification}/read`,{method:'POST'});if(el.dataset.link){location.href=el.dataset.link;}else renderNotifications();}catch(e){toast(e.message,'error');}});
}

async function renderProfile(){
  loading('profile');if(!await ensureAuth())return;
  const [account,notifications]=await Promise.all([api(`/points/accounts/${state.me.company_id}`),api('/notifications?page=1&page_size=100')]);
  const unreadCount=notifications.items.filter(item=>!item.read_at).length;
  zsSetSafeHtml(app, shell(`<h1 class="page-title">我的</h1><section class="card"><div class="brand"><img src="./logo.png" alt="合家美宅"><div>${esc(state.me.display_name)}<small>${esc(account.company_name)}</small></div></div><dl class="detail-list" style="margin-top:15px"><div class="detail-row"><dt>会员等级</dt><dd>${esc(memberLevelLabel(account))}</dd></div><div class="detail-row"><dt>所属公司</dt><dd>${esc(account.company_name||'合家美宅加盟商')}</dd></div></dl></section><section class="card" data-profile-messages><button class="btn btn-outline btn-block" data-route="notifications"><span>消息中心</span><small>${unreadCount} 条未读</small></button><button class="btn btn-outline btn-block" style="margin-top:10px" data-route="points">积分明细</button><button class="btn btn-danger btn-block" style="margin-top:10px" id="logout">退出登录</button></section><section class="card"><h3>隐私与安全</h3><p class="subtitle">领取成功后才会显示完整联系方式；退回证据采用私有存储和短时授权访问。</p></section>`,'profile','个人中心'));bindRoutes();document.querySelector('#logout').onclick=async()=>{try{await api('/auth/logout',{method:'POST'});}finally{state.me=null;location.replace('/admin/index.html');}};
}
