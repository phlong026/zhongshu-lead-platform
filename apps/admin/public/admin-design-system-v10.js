...
function admAddPageContext(){
  ...
  const crumb=document.createElement('div');crumb.className='adm-v10-breadcrumb';zsSetSafeHtml(crumb, `众墅之家 <span>›</span> <b>${ADM_META[id]?.[0]||'管理后台'}</b>`);page.insertBefore(crumb,head);
  const meta=ADM_META[id]||['管理后台','基于当前角色和权限显示数据','normal'];const scope=document.createElement('div');scope.className=`adm-v10-scope ${meta[2]==='risk'?'risk':meta[2]==='secure'?'secure':''}`;zsSetSafeHtml(scope, `<i>${meta[2]==='risk'?'!':'✓'}</i><span>${meta[1]}。页面、接口、数据范围和字段权限必须同时生效。</span>`);head.insertAdjacentElement('afterend',scope);
}
