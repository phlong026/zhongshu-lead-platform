...
async function telesales(){
  ...
  const rows=(data.items||[]).map(task=>{
    ...
    return `<tr><td><b>${esc(lead.customer_name||'待核验客户')}</b><br><small>${esc(lead.phone_masked||'--')}</small></td><td>${verificationTaskBadge(task)}</td><td>${esc(telesalesName(task.assignee_user_id))}</td><td>${esc(label(task.conclusion))}</td><td>${fmt(task.assigned_at)}</td><td>${action}</td></tr>`;
  });
  shell(`<section class="ops-card"><div class="ops-card-head"><div><h2>前置电销核验</h2><p>运营在此分配或改派；仅已提交结论的任务可由运营决定进入派发池、退回补充、标记重复或关闭。</p></div></div>${table(['客户','任务状态','电销人员','事实结论','派发时间','操作'],rows)}${pager(data)}</section>`);
