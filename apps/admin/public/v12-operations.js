...
const ADMIN_VIEW_CONTRACT={SUPER_ADMIN:['overview','leads','companies','finance','returns','audit'],OPERATION:['overview','leads','telesales','dispatch','companies','returns','audit']};
const ROLE_HOME_CONTRACT={SUPER_ADMIN:'平台管理员',OPERATION:'运营管理员'};
...
function shell(body){
  ...
  const pageSubtitle=S.view==='overview'&&roleHome?'当前身份的重点业务与待处理事项':S.view==='trace'?'查看客户从提交到处理完成的全部情况':'合家美宅统一工作台';
  const accountSettings=primaryRole()==='SUPER_ADMIN'?`<button class="ops-account-settings" data-account-settings type="button" aria-label="进入平台设置">${icon('settings')}<span>平台设置</span></button>`:'';
  zsSetSafeHtml(app, `<div class="ops-shell"><aside class="ops-side"><div class="ops-brand"><img class="ops-logo" src="./logo.png" alt="合家美宅"><div><strong>合家美宅统一工作台</strong><small>平台管理</small></div></div><div class="ops-menu-label">业务工作台</div><nav class="ops-menu">${nav()}</nav><div class="ops-account-zone"><button class="ops-account-card" data-account-center type="button"><i>${icon('user')}</i><span><b>${esc(S.me?.display_name||'当前账号')}</b><small>${esc(S.me?.username||ROLE_HOME_CONTRACT[primaryRole()]||'')}</small><em>${esc(ROLE_HOME_CONTRACT[primaryRole()]||'')}</em></span></button>${accountSettings}</div></aside><section class="ops-main"><header class="ops-top"><div class="ops-title"><h1>${esc(pageTitle)}</h1><p>${esc(pageSubtitle)}</p></div><div class="ops-top-actions"><button class="ops-btn primary" id="refresh">${icon('rotate-ccw')}刷新</button><button class="ops-btn" id="logout" type="button">${icon('log-out')}退出</button></div></header><main class="ops-content">${body}</main></section></div>`);
  document.querySelectorAll('[data-view]').forEach(button=>button.onclick=()=>go(button.dataset.view));
  document.querySelector('[data-account-center]')?.addEventListener('click',()=>go('account'));
  document.querySelector('[data-account-settings]')?.addEventListener('click',()=>go('settings'));
...
async function overview(){
  ...
  if(role==='SUPER_ADMIN'){
    ...
    const body=`<section class="ops-summary-columns"><div><h3>客资池</h3>${statusSummary({总量:pool.total||0,待派发:pool.unassigned||0,派发中:pool.dispatching||0,问题客资:pool.problem||0},'?view=leads')}</div><div><h3>电销与派发协同</h3>${statusSummary({待电销核验:verification.pending||0,核验中:verification.in_progress||0,待运营处置:verification.awaiting_operation||0,超时事项:verification.overdue||0},'?view=leads')}</div><div><h3>经营异常</h3>${statusSummary({退回待终审:exceptions.return_final_review||0,加盟商待审:exceptions.company_review||0,通知异常:exceptions.failed_notification||0,冻结奖励:funds.frozen_reward||0},'?view=returns')}</div></section><section class="ops-card"><div class="ops-card-head"><div><h2>平台治理提示</h2><p>首页只汇总经营状态和风险；日常初审、电销派发与人工派发由运营人员处理。</p></div></div>${table(['关注事项','数量','查看入口'],[['加盟商已停用',exceptions.disabled_company||0,'companies'],['通知发送异常',exceptions.failed_notification||0,'audit'],['冻结奖励',funds.frozen_reward||0,'finance']].map(([name,count,view])=>`<tr><td><b>${esc(name)}</b></td><td>${Number(count||0)}</td><td><button class="ops-btn" data-overview-view="${view}">查看</button></td></tr>`))}</section>`;
...
async function platformCities(){if(!S.platformCities){const tree=await api('/master-data/region-tree');S.platformCities=(tree.provinces||[]).flatMap(province=>(province.cities||[]).map(city=>({...city,province_name:province.name,option_name:`${province.name} · ${city.name}`})));}return S.platformCities}
...
async function account(){
  const role=ROLE_HOME_CONTRACT[primaryRole()]||'平台账号';
  shell(`<section class="ops-card"><div class="ops-card-head"><div><h2>账号中心</h2><p>查看当前登录身份并维护个人登录安全；平台设置与个人账号分开管理。</p></div></div><div class="ops-account-detail"><i>${icon('user')}</i><div><h3>${esc(S.me?.display_name||'当前账号')}</h3><p>${esc(S.me?.username||'--')} · ${esc(role)}</p></div></div><div class="ops-detail-grid">${[['登录账号',S.me?.username||'--'],['当前角色',role],['所属组织',S.me?.company_name||'合家美宅平台'],['账号状态','已登录']].map(([name,value])=>`<div class="ops-detail"><small>${esc(name)}</small><b>${esc(value)}</b></div>`).join('')}</div><div class="ops-actions ops-account-actions"><button class="ops-btn" id="account-password" type="button">修改登录密码</button><button class="ops-btn" id="account-logout" type="button">${icon('log-out')}退出当前账号</button>${isSuperAdmin()?'<button class="ops-btn primary" id="account-settings" type="button">进入平台设置</button>':''}</div></section>`);
  ...
  document.querySelector('#account-settings')?.addEventListener('click',()=>go('settings'));
}
...
async function companies(){
  ...
  const companyRows=(companyPage.items||[]).map(company=>`<tr><td><b>${esc(company.name)}</b><br><small>${esc(company.code)}</small></td><td>${badge(company.status)}</td><td>${companyAssignmentSummary(company.assignment_summary)}</td><td>${esc(company.owner_name||'--')}</td><td><button class="ops-btn" data-company-edit="${esc(company.id)}">编辑资料</button> <button class="ops-btn primary" data-company-accounts="${esc(company.id)}" data-company-name="${esc(company.name)}">账号与人员</button>${pendingProfileAction(company)}</td></tr>`);
