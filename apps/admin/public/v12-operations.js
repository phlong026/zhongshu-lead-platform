...
function shell(body){
  ...
  zsSetSafeHtml(app, `<div class="ops-shell"><aside class="ops-side"><div class="ops-brand"><img class="ops-logo" src="./logo.png" alt="合家美宅"><div><strong>合家美宅</strong><small>客资运营台</small></div></div><div class="ops-menu-label">业务运营</div><nav class="ops-menu">${nav()}</nav>${systemSection}<div class="ops-side-foot">${esc(S.me?.display_name||'')}</div></aside><section class="ops-main"><header class="ops-top"><div class="ops-title"><h1>${esc(pageTitle)}</h1><p>${esc(pageSubtitle)}</p></div><div class="ops-top-actions">${shortcut}<button class="ops-btn primary" id="refresh">${icon('rotate-ccw')}刷新</button></div></header><main class="ops-content">${body}</main></section></div>`);
...
async function returnReviewerHome(){
  ...
  const items=data.items||[];
  const rows=items.slice(0,8).map(item=>`<tr><td>${esc(item.id)}</td><td>${esc(label(item.reason_code))}</td><td>${badge(item.status)}</td><td>${fmt(item.submitted_at||item.created_at)}</td><td><button class="ops-btn" data-return-home="${esc(item.id)}">查看审核</button></td></tr>`);
