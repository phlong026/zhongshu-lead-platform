...
async function returnReviewerHome(){
  ...
  roleHome(ADMIN_ROLE_HOME_CONTENT.RETURN_REVIEWER,[
    ['待终审',countStatus(items,['REVIEWING']),'clipboard-check','?view=returns&status=REVIEWING'],
    ['待补证',countStatus(items,['NEED_MORE_EVIDENCE']),'file-text','?view=returns&status=NEED_MORE_EVIDENCE'],
    ['已完成核验',countStatus(items,['APPROVED','REJECTED']),'user-check','?view=returns'],
    ['截止提醒',items.filter(item=>item.appeal_deadline_at).length,'calendar','?view=returns'],
...
function platformRoleHome(role,report,summary,alerts){
  ...
  let cards=[];
  if(role==='SUPER_ADMIN')cards=[['风险预警',alertTotal,'alert-triangle','./index.html#/dashboard'],['账号角色','管理','users','./index.html#/users'],['加盟商配置',business.active_companies||0,'building','./index.html#/companies'],['积分规则','查看','coins','./index.html#/points'],['工作日历','查看','calendar','./index.html#/calendar'],['通知失败',alerts.failed_notifications||0,'bell','./index.html#/outbox'],['高风险审计','查看','search','./index.html#/audit']];
  else if(role==='OWNER')cards=[['客资漏斗',business.total_leads||report.leads.total,'list','./v12-leads.html'],['派发领取',business.claimed_total||0,'hand-claim','?view=dispatch'],['跟进率',`${business.followup_rate||0}%`,'phone','./index.html#/assignments'],['成交率',`${business.conversion_rate||0}%`,'award','./index.html#/assignments'],['退回申诉',report.returns.total,'rotate-ccw','?view=returns'],['积分变化',report.points_ledger?.net_delta??0,'activity','./index.html#/ledgers'],['异常提醒',alertTotal,'alert-triangle','./index.html#/dashboard']];
  ...
  else cards=[['待初审',report.leads.by_status?.PENDING_REVIEW||0,'user-check','?view=review'],['待派发',report.leads.by_status?.READY_DISPATCH||0,'hand-claim','?view=dispatch'],['待分配电销',report.returns.by_status?.VERIFYING||0,'phone','?view=returns&status=VERIFYING'],['待终审',report.returns.by_status?.REVIEWING||0,'clipboard-check','?view=returns&status=REVIEWING'],['能力区域待审','进入审核','building','?view=companies'],['通知异常',alerts.failed_notifications||0,'bell','./index.html#/outbox']];
  const aggregate=role==='FINANCE'?'':`<section class="ops-card"><div class="ops-card-head"><div><h2>业务状态</h2><p>点击状态数字进入对应筛选明细。</p></div></div><div class="ops-summary-columns"><div><h3>客资</h3>${statusSummary(report.leads.by_status,status=>`./v12-leads.html?status=${encodeURIComponent(status)}`)}</div><div><h3>派发</h3>${statusSummary(report.assignments.by_status,status=>`./index.html#/assignments?status=${encodeURIComponent(status)}`)}</div><div><h3>退回</h3>${statusSummary(report.returns.by_status,status=>`?view=returns&status=${encodeURIComponent(status)}`)}</div></div></section>`;
...
async function returns(){const [d,t]=await Promise.all([api(`/v1.2/returns${qs({status:S.status,page:S.page,page_size:20})}`),can('verification.read')?api('/v1.2/return-verifications/tasks?page=1&page_size=100'):Promise.resolve({items:[]})]);if(can('verification.read'))await loadTelesalesUsers();const rows=(d.items||[]).map(x=>`<tr><td>${esc(recordCode(x.id,'TH'))}<br><small>派发编号 ${esc(recordCode(x.assignment_id,'PF'))}</small></td><td>${esc(label(x.reason_code))}</td><td>${badge(x.status)}</td><td>${fmt(x.submitted_at||x.created_at)}</td><td><button class="ops-btn" data-return="${x.id}">查看与审核</button></td></tr>`);const tasks=(t.items||[]).map(x=>{const r=x.return_request||{},lead=x.lead||{};return `<tr><td><b>${esc(lead.customer_name||'待核验客户')}</b><br><small>${esc(lead.phone_masked||'--')}</small></td><td>${verificationTaskBadge(x)}</td><td>${esc(telesalesName(x.assignee_user_id))}</td><td>${esc(label(r.reason_code))}</td><td><button class="ops-btn" data-task="${x.id}">查看</button> <button class="ops-btn" data-assign="${x.id}">${x.assignee_user_id?'重新分配':'分配人员'}</button></td></tr>`});const filterNotice=S.status?`<div class="ops-notice">当前筛选：${esc(label(S.status))} <button class="ops-btn" id="returns-clear">查看全部</button></div>`:'';shell(`${filterNotice}<section class="ops-card"><h2>退回申诉</h2>${table(['退回编号','退回原因','处理状态','申诉时间','操作'],rows)}${pager(d)}</section>${can('verification.read')?`<section class="ops-card"><h2>电话核验任务</h2><p>仅在加盟商发起退回申诉后进行电话核验。</p>${table(['客户','状态','核验人员','退回原因','操作'],tasks)}</section>`:''}`);bindPager(d,returns);document.querySelector('#returns-clear')?.addEventListener('click',()=>go('returns'));document.querySelectorAll('[data-return]').forEach(b=>b.onclick=()=>returnDetail(b.dataset.return));document.querySelectorAll('[data-task]').forEach(b=>b.onclick=()=>taskDetail(b.dataset.task));document.querySelectorAll('[data-assign]').forEach(b=>b.onclick=()=>assignTask(b.dataset.assign));if(S.id){const id=S.id;S.id='';returnDetail(id)}}
