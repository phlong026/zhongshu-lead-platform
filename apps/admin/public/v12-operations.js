...
function evidenceList(items){return (items||[]).map(item=>`<article class="ops-detail"><small>${esc(EVIDENCE_LABEL[item.type]||'申诉证据')}</small><b>${esc(item.original_name||'未命名文件')}</b><p>${esc(fileSize(item.file_size))} · ${fmt(item.created_at)}</p>${item.access_token?`<a class="ops-btn" target="_blank" rel="noopener" href="${API}/v1.2/return-evidences/${encodeURIComponent(item.id)}/download?token=${encodeURIComponent(item.access_token)}">查看证据</a>`:''}</article>`).join('')||'<div class="ops-empty">暂无证据</div>'}
...
async function overview(){const d=await api('/v1.2/reports/overview');const k=[['客资总量',d.leads.total,'list'],['派发单',d.assignments.total,'hand-claim'],['退回申诉',d.returns.total,'rotate-ccw'],['供应奖励',d.supplier_rewards.points+' 分','award'],['积分流水',d.points_ledger.count,'coins'],['净积分变化',d.points_ledger.net_delta,'activity']];shell(`<div class="ops-grid">${k.map(x=>`<div class="ops-kpi"><i>${icon(x[2])}</i><small>${x[0]}</small><b>${x[1]}</b></div>`).join('')}</div><section class="ops-card"><h2>客资状态</h2>${statusSummary(d.leads.by_status)}</section><section class="ops-card"><h2>派发状态</h2>${statusSummary(d.assignments.by_status)}</section><section class="ops-card"><h2>退回状态</h2>${statusSummary(d.returns.by_status)}</section><section class="ops-card"><h2>奖励状态</h2>${statusSummary(d.supplier_rewards.by_status)}</section>`)}
...
async function candidates(leadId){const d=await api(`/v1.2/dispatch-pool/${encodeURIComponent(leadId)}/candidates`);const rows=(d.candidates||[]).map(x=>`<tr><td>${esc(x.company_name)}</td><td>${x.eligible?badge('APPROVED'):badge('REJECTED')}</td><td>${x.points_price}</td><td>${x.points_available??'按权限隐藏'}</td><td>${esc((x.exclusion_reasons||[]).join('、')||'符合条件')}</td><td>${x.eligible?`<button class="ops-btn primary" data-dispatch="${x.company_id}">派发</button>`:'--'}</td></tr>`);modal('选择接收公司',table(['接收公司','是否可派','所需积分','可用积分','判断说明','操作'],rows),()=>document.querySelectorAll('[data-dispatch]').forEach(b=>b.onclick=()=>dispatchOne(leadId,b.dataset.dispatch)))}
...
async function rewards(){const [d,r]=await Promise.all([api(`/v1.2/supplier-rewards${qs({page:S.page,page_size:20})}`),can('reward.manage')?api('/v1.2/admin/supplier-reward-rules/current'):Promise.resolve(null)]);const rows=(d.items||[]).map(x=>`<tr><td>${esc(x.id)}<br><small>派发单 ${esc(x.assignment_id)}</small></td><td>${esc(x.supplier_company_id)}</td><td>${x.claim_points}</td><td>${x.reward_points}</td><td>${badge(x.status)}</td><td>${fmt(x.reward_due_at)}</td><td><button class="ops-btn" data-reward="${x.id}">查看</button>${can('reward.manage')&&x.status==='OBSERVING'?` <button class="ops-btn primary" data-settle="${x.id}">结算</button>`:''}${can('reward.reverse')&&x.status==='SETTLED'?` <button class="ops-btn danger" data-reverse="${x.id}">撤销奖励</button>`:''}</td></tr>`);shell(`${r?`<section class="ops-card"><h2>当前奖励规则</h2>${ruleSummary(r)}<div class="ops-actions"><button class="ops-btn" id="new-rule">调整奖励比例</button><button class="ops-btn gold" id="settle-due">结算已到期奖励</button></div></section>`:''}<section class="ops-card"><h2>供应商奖励</h2>${table(['奖励编号','供应商','领取积分','奖励积分','状态','预计结算','操作'],rows)}${pager(d)}</section>`);bindPager(d,rewards);document.querySelectorAll('[data-reward]').forEach(b=>b.onclick=()=>rewardDetail(b.dataset.reward));document.querySelectorAll('[data-settle]').forEach(b=>b.onclick=()=>settle(b.dataset.settle));document.querySelectorAll('[data-reverse]').forEach(b=>b.onclick=()=>reverse(b.dataset.reverse));document.querySelector('#settle-due')?.addEventListener('click',settleDue);document.querySelector('#new-rule')?.addEventListener('click',()=>newRule(r));if(S.id){const id=S.id;S.id='';rewardDetail(id)}}
async function rewardDetail(id){const x=await api(`/v1.2/supplier-rewards/${encodeURIComponent(id)}`);modal('奖励详情',`<div class="ops-detail-grid">${[['奖励编号',x.id],['派发单',x.assignment_id],['供应商',x.supplier_company_id],['接收公司',x.receiver_company_id],['状态',label(x.status)],['领取积分',x.claim_points],['奖励积分',x.reward_points],['预计结算',fmt(x.reward_due_at)],['实际到账',fmt(x.settled_at)]].map(([a,b])=>`<div class="ops-detail"><small>${a}</small><b>${esc(b||'--')}</b></div>`).join('')}</div><section class="ops-card"><h3>本笔奖励适用规则</h3>${ruleSummary(x.rule_snapshot||{})}</section><button class="ops-btn" id="trace">查看业务记录</button>`,()=>document.querySelector('#trace').onclick=()=>{closeModal();go('audit',id)})}
...
async function newRule(currentRule){const currentRatio=Number(currentRule?.ratio_bps||0)/100;const input=prompt('奖励比例（%）',String(currentRatio));if(input===null)return;const ratio=Number(input);if(!Number.isFinite(ratio)||ratio<=0||ratio>100)return toast('请输入 0 到 100 之间的奖励比例',true);try{await api('/v1.2/admin/supplier-reward-rules',{method:'POST',body:JSON.stringify({ratio_bps:Math.round(ratio*100),min_points:currentRule.min_points,max_points:currentRule.max_points,hard_duplicate_days:currentRule.hard_duplicate_days,reward_duplicate_days:currentRule.reward_duplicate_days,historical_suspect_days:currentRule.historical_suspect_days,publish_immediately:true})});toast('奖励比例已更新');rewards()}catch(e){toast(e.message,true)}}
async function audit(){const business=S.id||'';const d=await api(`/v1.2/audit-events${qs({page:S.page,page_size:50,business_id:business})}`);const rows=(d.items||[]).map(x=>`<tr><td>${fmt(x.created_at)}</td><td>${esc(auditAction(x.action))}<br><small>操作人编号：${esc(x.actor_user_id||'系统自动处理')}</small></td><td>${esc(auditResource(x.resource_type))}<br><small>业务记录：${esc(x.resource_id||'--')}</small></td><td>${esc(x.company_id||'--')}</td><td>${esc(x.request_id||'--')}</td></tr>`);shell(`<div class="ops-filter"><input class="ops-input" id="business" placeholder="输入客资、派发单或申诉编号" value="${esc(business)}"><button class="ops-btn primary" id="query">查询操作记录</button><button class="ops-btn gold" id="trace" ${business?'':'disabled'}>查看完整记录</button></div><section class="ops-card"><h2>操作记录</h2>${table(['时间','处理动作','业务对象','加盟商编号','操作记录编号'],rows)}${pager(d)}</section>`);bindPager(d,audit);document.querySelector('#query').onclick=()=>go('audit',document.querySelector('#business').value.trim());document.querySelector('#trace').onclick=()=>trace(document.querySelector('#business').value.trim());if(S.id){const id=S.id;S.id='';trace(id)}}
async function trace(id){if(!id)return;try{const d=await api(`/v1.2/trace/${encodeURIComponent(id)}`);const timeline=(d.timeline||[]).map(item=>`<tr><td>${fmt(item.at)}</td><td>${esc(auditAction(item.action))}</td><td>${esc(auditResource(item.resource_type))}</td><td>${esc(item.resource_id||item.id||'--')}</td></tr>`);modal('完整业务记录',`<div class="ops-detail-grid">${[['查询编号',d.business_id],['关联记录',d.linked_ids?.length],['派发单',d.assignments?.length],['退回申诉',d.returns?.length],['供应奖励',d.supplier_rewards?.length],['电话核验',d.verification_tasks?.length],['消息',d.notifications?.length],['操作记录',d.audit_events?.length]].map(([a,b])=>`<div class="ops-detail"><small>${a}</small><b>${esc(b??0)}</b></div>`).join('')}</div><section class="ops-card"><h3>处理时间线</h3>${table(['时间','处理动作','业务对象','记录编号'],timeline)}</section>`)}catch(e){toast(e.message,true)}}
