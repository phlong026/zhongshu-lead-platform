...
async function companyDetail(company){
  ...
  modal(`${detail.name} · 加盟商信息`,`<section class="ops-company-detail ops-company-detail-compact"><div class="ops-company-facts"><div><small>主体状态</small><b>${esc(label(detail.status))}</b></div><div><small>负责人</small><b>${esc(detail.owner_name||'未填写')}</b></div><div><small>联系电话</small><b>${esc(detail.contact_phone_masked||'未填写')}</b></div><div><small>负责人微信</small><b>${detail.wechat_bound?'已绑定':'待绑定'}</b></div></div><div class="ops-company-summary-grid"><section class="ops-company-section"><div class="ops-company-section-head"><div><h3>服务区域</h3><p>已开通 ${areas.length} 个区/县。</p></div></div><div class="ops-area-chips">${areaCards}</div>${areaDetail}</section><section class="ops-company-section"><div class="ops-company-section-head"><div><h3>客资功能</h3><p>平台统一配置。</p></div></div><div class="ops-company-capabilities compact">${['LEAD_RECEIVER','LEAD_SUPPLIER'].map(code=>companyCapabilitySetting(profile.capabilities,code,true)).join('')}</div></section></div><section class="ops-company-binding-compact"><div><small>负责人绑定</small><b>${detail.wechat_bound?'已完成绑定':'等待负责人绑定'}</b><p>${esc(inviteNotice)}</p></div><div class="ops-actions">${!detail.wechat_bound&&detail.status==='ACTIVE'?`<button class="ops-btn primary" data-invite-create="${esc(detail.id)}">发起绑定</button>`:''}<button class="ops-btn" data-company-detail-accounts="${esc(detail.id)}">账号与人员</button><button class="ops-btn" data-company-detail-edit="${esc(detail.id)}">编辑资料</button></div></section>${lifecycleActions?`<section class="ops-company-binding-compact"><div><small>主体治理</small><b>停用只负责业务隔离</b><p>解绑用于负责人微信重新关联；删除仅清理无业务数据的测试加盟商。</p></div><div class="ops-actions">${lifecycleActions}</div></section>`:''}${inviteHistory}</section>`,()=>{
...
function markCompanyAsTest(company){
  companyLifecycleConfirmation({title:'标记历史测试数据',message:'仅用于过去创建但未标记的联测主体。系统会再次检查是否有客资、派发、积分、退回、跟进或奖励记录。',company,submitLabel:'确认标记为测试'},async body=>{
...
function deleteTestCompany(company){
  companyLifecycleConfirmation({title:'删除测试数据',message:'删除仅用于清理已停用、无业务数据的测试加盟商。正常主体或已有业务记录的主体不会被删除。',company,submitLabel:'确认删除测试数据'},async body=>{
