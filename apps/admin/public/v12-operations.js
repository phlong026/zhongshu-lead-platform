...
const S={me:null,view:'overview',id:'',status:'',page:1,processedPage:1,processedCreatedFrom:beijingToday(),processedCreatedTo:beijingToday(),leadSource:'',leadCreatedFrom:'',leadCreatedTo:'',leadStatusFilter:'',assignmentStatusFilter:'',leadAssignerId:'',leadSubmitterId:'',leadPhone:'',leadRegion:'',leadReceiverCompanyId:'',leadFilterOptions:null,operationUsers:null,platformLeads:[],supplierLeads:[],publicPoolKeyword:'',publicPoolSource:'',publicPoolCompleteness:'',publicPoolDuplicate:'',financeRewardPage:1,financeCompanyKeyword:'',financeCompanyStatus:'',financeCompanyPage:1,financeCompanyId:'',financeLedgerType:'',financeDays:30,financeRewardStatus:'',financeSource:'',platformCities:null,platformDistricts:[],companyKeyword:'',companyLifecycleStatus:'',companyPage:1,telesalesUsers:null,calendarMonth:'',unreadNotifications:0,accountNotifications:[]};
...
async function transferPublicPoolLead(item){
  try{
    ...
    modal('暂不能进入派发池',`<div class="ops-notice">${errors.map(esc).join('；')||'请先处理手机号查重结论'}</div><div class="ops-actions"><button class="ops-btn primary" id="public-pool-edit-now">立即补充资料</button></div>`,()=>document.querySelector('#public-pool-edit-now').onclick=()=>{closeModal();openPlatformLeadForm(item)});
...
async function publicPool(){
  const data=await api(`/v1.2/public-pool/leads${qs({page:S.page,page_size:20,keyword:S.publicPoolKeyword||undefined,source_kind:S.publicPoolSource||undefined,completeness:S.publicPoolCompleteness||undefined,duplicate_status:S.publicPoolDuplicate||undefined})}`);
  const rows=(data.items||[]).map(item=>{
    ...
    return `<tr><td><b>${esc(item.customer_name||'未填写')}</b><br><small>${esc(item.phone_masked||'--')}</small></td><td>${badge(item.source_kind)}</td><td>${esc(item.city||'待补充')} ${esc(item.district||'')}</td><td>${esc(item.source_display||label(item.source_channel)||'待补充')}</td><td>${esc(publicPoolValidationText(item))}</td><td>${badge(item.duplicate_status||'CLEAR')}</td><td>${esc(item.submitter_name||'后台人员')}<br><small>${fmt(item.created_at)}</small></td><td>${actions.join(' ')}</td></tr>`;
  });
  ...
  shell(`<section class="ops-card"><div class="ops-card-head"><div><h2>公海池</h2><p>运营及管理人员共享维护；资料不完整或查重待处理的客户先留在这里，转入派发池时系统会重新完整校验。</p></div><div class="ops-actions"><button class="ops-btn primary" id="public-pool-new">新增客户</button><button class="ops-btn" id="public-pool-inline">直接添加一行</button><button class="ops-btn gold" id="public-pool-feishu">从飞书客户视图导入</button></div></div><form class="ops-filter" id="public-pool-filter"><input class="ops-input" id="public-pool-keyword" value="${esc(S.publicPoolKeyword)}" placeholder="客户、地区或具体来源"><select class="ops-input" id="public-pool-source"><option value="">全部录入方式</option><option value="PLATFORM_MANUAL" ${S.publicPoolSource==='PLATFORM_MANUAL'?'selected':''}>后台录入</option><option value="FEISHU_IMPORT" ${S.publicPoolSource==='FEISHU_IMPORT'?'selected':''}>飞书导入</option></select><select class="ops-input" id="public-pool-completeness"><option value="">全部完整性</option><option value="COMPLETE" ${S.publicPoolCompleteness==='COMPLETE'?'selected':''}>资料可提交</option><option value="INCOMPLETE" ${S.publicPoolCompleteness==='INCOMPLETE'?'selected':''}>资料待补充</option></select><select class="ops-input" id="public-pool-duplicate"><option value="">全部查重状态</option><option value="CLEAR" ${S.publicPoolDuplicate==='CLEAR'?'selected':''}>未发现重复</option><option value="HARD_DUPLICATE" ${S.publicPoolDuplicate==='HARD_DUPLICATE'?'selected':''}>短期重复</option><option value="HISTORICAL_SUSPECT" ${S.publicPoolDuplicate==='HISTORICAL_SUSPECT'?'selected':''}>历史疑似</option></select><button class="ops-btn primary" type="submit">查询</button></form></section>${inlineForm}<section class="ops-card"><div class="ops-card-head"><div><h2>待处理客户</h2><p>加盟商看不到本页和其中的客户信息。</p></div></div>${table(['客户','录入方式','所在地','来源','完整性','手机号查重','创建人 / 时间','操作'],rows)}${pager(data)}</section>`);
  ...
  document.querySelector('#public-pool-filter').onsubmit=event=>{event.preventDefault();S.publicPoolKeyword=document.querySelector('#public-pool-keyword').value.trim();S.publicPoolSource=document.querySelector('#public-pool-source').value;S.publicPoolCompleteness=document.querySelector('#public-pool-completeness').value;S.publicPoolDuplicate=document.querySelector('#public-pool-duplicate').value;S.page=1;publicPool()};
...
async function savePlatformLead(item,correction=false){
  ...
  try{
    ...
    closeModal();toast(correction?'客资更正已保存，异常会标记待运营处理':S.view==='publicPool'?'客户已保存到公海池':'平台客资草稿已保存');await (S.view==='publicPool'?publicPool():review())
  }catch(error){toast(error.message,true)}
}
