...
async function audit(){const business=S.id||'';const d=await api(`/v1.2/audit-events${qs({page:S.page,page_size:50,business_id:business})}`);const rows=(d.items||[]).map(x=>`<tr><td>${fmt(x.created_at)}</td><td>${esc(auditAction(x.action))}<br><small>${x.actor_user_id?`操作账号：${esc(recordCode(x.actor_user_id,'账号'))}`:'系统自动处理'}</small></td><td>${esc(auditResource(x.resource_type))}<br><small>相关编号：${esc(recordCode(x.resource_id,'业务'))}</small></td><td>${esc(recordCode(x.company_id,'加盟商'))}</td><td>${esc(recordCode(x.request_id,'操作'))}</td></tr>`);shell(`<div class="ops-filter"><input class="ops-input" id="business" placeholder="输入客户、客资、派发或退回编号" value="${esc(business)}"><button class="ops-btn primary" id="query">查询记录</button><button class="ops-btn gold" id="trace" ${business?'':'disabled'}>查看客资详情</button></div><section class="ops-card"><h2>操作日志</h2>${table(['时间','处理动作','相关事项','加盟商','操作编号'],rows)}${pager(d)}</section>`);bindPager(d,audit);document.querySelector('#query').onclick=()=>go('audit',document.querySelector('#business').value.trim());document.querySelector('#trace').onclick=()=>go('trace',document.querySelector('#business').value.trim())}
