...
async function companies(){
  ...
  const companyRows=(companyPage.items||[]).map(company=>{const active=company.status==='ACTIVE',lifecycle=active?'disable':'enable',lifecycleLabel=active?'停用':'启用',deleteAction=company.status==='DISABLED'&&company.is_test?` <button class="ops-btn danger" data-company-delete="${esc(company.id)}">删除测试主体</button>`:'';return `<tr><td><button class="ops-company-link" data-company-detail="${esc(company.id)}" type="button">${esc(company.name)}</button><br><small>${esc(company.code)}${company.is_test?' · 测试主体':''}</small></td><td>${badge(company.status)}</td><td>${companyAssignmentSummary(company.assignment_summary)}</td><td>${esc(company.owner_name||'--')}</td><td><button class="ops-btn ${active?'danger':'primary'}" data-company-lifecycle="${esc(company.id)}:${lifecycle}">${lifecycleLabel}</button> <button class="ops-btn primary" data-company-accounts="${esc(company.id)}" data-company-name="${esc(company.name)}">账号与人员</button>${deleteAction}</td></tr>`});
  ...
  document.querySelectorAll('[data-company-delete]').forEach(button=>button.onclick=()=>deleteListedTestCompany(companiesById[button.dataset.companyDelete]));
}
...
function deleteListedTestCompany(company){
  if(!company)return;
  deleteTestCompany(company);
}
...
async function companyDetail(company){
  ...
  const lifecycleActions=`${detail.status==='ACTIVE'?`<button class="ops-btn danger" data-company-disable="${esc(detail.id)}">停用主体</button>`:''}${detail.status==='DISABLED'?`<button class="ops-btn primary" data-company-enable="${esc(detail.id)}">重新启用</button>`:''}${detail.status==='DISABLED'&&detail.wechat_bound?`<button class="ops-btn danger" data-company-wechat-unbind="${esc(detail.id)}">解绑负责人微信</button>`:''}${detail.status==='DISABLED'&&!detail.is_test&&isSuperAdmin()?`<button class="ops-btn danger" data-company-mark-test="${esc(detail.id)}">标记历史测试数据</button>`:''}${detail.status==='DISABLED'&&detail.is_test?`<button class="ops-btn danger" data-company-test-delete="${esc(detail.id)}">删除测试数据</button>`:''}`;
  modal(`${detail.name} · 加盟商信息`,`<section class="ops-company-detail ops-company-detail-compact"><div class="ops-company-facts"><div><small>主体状态</small><b>${esc(label(detail.status))}</b></div><div><small>负责人</small><b>${esc(detail.owner_name||'未填写')}</b></div><div><small>联系电话</small><b>${esc(detail.contact_phone_masked||'未填写')}</b></div><div><small>负责人微信</small><b>${detail.wechat_bound?'已绑定':'待绑定'}</b></div></div><div class="ops-company-summary-grid"><section class="ops-company-section"><div class="ops-company-section-head"><div><h3>服务区域</h3><p>已开通 ${areas.length} 个区/县。</p></div></div><div class="ops-area-chips">${areaCards}</div>${areaDetail}</section><section class="ops-company-section"><div class="ops-company-section-head"><div><h3>客资功能</h3><p>平台统一配置。</p></div></div><div class="ops-company-capabilities compact">${['LEAD_RECEIVER','LEAD_SUPPLIER'].map(code=>companyCapabilitySetting(profile.capabilities,code,true)).join('')}</div></section></div><section class="ops-company-binding-compact"><div><small>负责人绑定</small><b>${detail.wechat_bound?'已完成绑定':'等待负责人绑定'}</b><p>${esc(inviteNotice)}</p></div><div class="ops-actions">${!detail.wechat_bound&&detail.status==='ACTIVE'?`<button class="ops-btn primary" data-invite-create="${esc(detail.id)}">发起绑定</button>`:''}<button class="ops-btn" data-company-detail-accounts="${esc(detail.id)}">账号与人员</button><button class="ops-btn" data-company-detail-edit="${esc(detail.id)}">编辑资料</button></div></section>${lifecycleActions?`<section class="ops-company-binding-compact"><div><small>主体治理</small><b>停用只负责业务隔离</b><p>解绑用于负责人微信重新关联；删除会完整清理已停用测试主体的业务与资金数据。</p></div><div class="ops-actions">${lifecycleActions}</div></section>`:''}${inviteHistory}</section>`,()=>{
...
function markCompanyAsTest(company){
  companyLifecycleConfirmation({title:'标记历史测试数据',message:'仅用于过去创建但未标记的联测主体。标记后将允许完整删除该主体的业务与资金数据；如果与平台或其他主体存在交叉业务，系统会拒绝标记。',company,submitLabel:'确认标记为测试'},async body=>{
...
function deleteTestCompany(company){
  companyLifecycleConfirmation({title:'删除测试数据',message:'该操作会永久删除测试主体、积分账户、充值与积分流水、客资、派发、退回、跟进、奖励和业务消息。原成员账号会停用并解除公司与微信关联，审计记录会保留。如果与平台或其他主体有交叉业务，系统会拒绝删除。',company,submitLabel:'确认永久删除'},async body=>{
