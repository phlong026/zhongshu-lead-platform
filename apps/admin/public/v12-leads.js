...
function shell(content=''){
  app.innerHTML=`<div class="v12-shell"><header class="v12-topbar"><div class="v12-brand"><img src="./logo.png" alt="众墅之家"><div><strong>V1.2 客资供给工作台</strong><small>平台录入 · 供应商初审 · 人工派发前准备</small></div></div><div class="v12-top-actions"><span>${esc(state.me?.display_name||'')}</span><a class="v12-btn" href="./#/dashboard">返回管理后台</a></div></header><main class="v12-main"><section class="v12-hero"><div><h1>客资供给与资料初审</h1><p>正常客资完成字段、授权与去重校验后直接进入待人工派发池。本页面不会生成前置电销任务。</p></div></section><nav class="v12-tabs">${can('lead.manual.manage')?`<button class="v12-tab ${state.tab==='platform'?'active':''}" data-tab="platform">平台手工录入</button>`:''}${can('lead.supplier.review')?`<button class="v12-tab ${state.tab==='supplier'?'active':''}" data-tab="supplier">供应商资料初审</button>`:''}</nav><div id="workspace">${content}</div></main></div>`;
...
function fail(error){console.error(error);document.querySelector('#workspace').innerHTML=`<div class="v12-error"><b>页面加载失败</b><div style="margin-top:8px">${esc(error.message||'未知错误')}</div></div>`;toast(error.message||'页面加载失败','error');}
function table(headers,rows,emptyText){return rows.length?`<div class="v12-table-wrap"><table class="v12-table"><thead><tr>${headers.map(item=>`<th>${esc(item)}</th>`).join('')}</tr></thead><tbody>${rows.join('')}</tbody></table></div>`:`<div class="v12-empty"><b>⌕</b>${esc(emptyText)}</div>`;}

async function boot(){
  try{
    state.me=await request('/auth/me');
    if(!can('lead.manual.manage')&&!can('lead.supplier.review'))throw new Error('当前账号无权访问 V1.2 客资供给工作台');
    ...
    app.innerHTML=`<main class="v12-main"><div class="v12-error">${esc(error.message||'无法进入工作台')}</div><p><a class="v12-btn" href="./#/dashboard">返回管理后台</a></p></main>`;
  }
}
...
async function openLeadForm(item){
  ...
  document.querySelector('#lead-city').onchange=async event=>{await loadDistricts(event.target.value);const select=document.querySelector('#lead-district');select.innerHTML='<option value="">全市范围</option>'+state.districts.map(row=>`<option value="${row.code}">${esc(row.name)}</option>`).join('');};
...
async function submitPlatformLead(id){
  ...
  openModal('提交客资',`<p>提交后将执行字段、授权和手机号去重校验。校验通过的正常客资会直接进入待人工派发池。</p><div class="v12-note">不会生成前置电销核验任务。重复或历史疑似客资将进入人工复核。</div>`,`<button data-close class="v12-btn">取消</button><button id="confirm-submit" class="v12-btn primary">确认提交</button>`);
