...
def main() -> int:
    ...
    with sync_playwright() as playwright:
        ...
        try:
            admin = _admin_smoke(browser, args.base_url.rstrip("/"), args.output_dir, errors)
            h5 = _h5_smoke(browser, args.base_url.rstrip("/"), args.output_dir, errors)
        finally:
            browser.close()
    # Browser extensions and favicon requests can create console noise, but any
    # application page error or console error is a release-gate failure.
    payload = {"admin": admin, "h5": h5, "errors": errors, "valid": not errors}
