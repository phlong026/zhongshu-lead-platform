...
已停用且明确标记为测试的主体可以执行完整清理。清理范围包括加盟商主体、积分账户、充值/调账/冲正流水、自有客资、派发、退回、跟进、奖励、邀请和业务消息。如果与平台客资、其他加盟商或外部积分流水存在交叉关联，服务端会拒绝标记或删除。审计记录始终保留，原成员账号会立即停用并解除公司/微信关联。

删除时必须在弹窗中准确输入加盟商完整名称、填写操作原因并二次确认。历史上未标记的测试主体只能由超级管理员在同样的确认后标记；正常加盟商不能直接删除。

| 方法 | 路径 | 用途 |
|---|---|---|
| `PATCH` | `/api/v1/companies/{company_id}` | 通过 `status` 和 `reason` 停用或重新启用加盟商主体 |
| `POST` | `/api/v1/companies/{company_id}/mark-test` | 超级管理员标记历史测试主体 |
| `DELETE` | `/api/v1/companies/{company_id}` | 完整清理已停用测试主体，Body 必须包含 `confirm_name` 和 `reason` |
